...
class AudioEngine {
	...
	private async resolveAudioPath(path: string): Promise<string> {
		...
		// 如果是绝对文件路径，转换为 Obsidian 的 app://local 协议
		if (path.match(/^[A-Za-z]:\\/)) {
			const normalizedPath = path.replace(/\\/g, '/');
			return `app://local/${normalizedPath}`;
		}
		
		// 处理相对路径 - 尝试使用 Obsidian 的文件 API
		try {
			// 首先检查文件是否存在
			const file = this.app.vault.getAbstractFileByPath(path);
			if (file && file instanceof TFile) {
				// 使用 Obsidian API 读取文件并创建 blob URL
		...
		if (adapter && 'basePath' in adapter) {
			...
			console.log(`Using basePath: ${vaultPath}`);
		}
		
		if (vaultPath) {
			const fullPath = `${vaultPath}/${path}`.replace(/\\/g, '/');
			console.log(`Vault path: ${vaultPath}, Full path: ${fullPath}`);
	...
	async play(track: MusicTrack): Promise<void> {
		try {
			...
			console.log(`Attempting to play: ${track.path} -> ${resolvedPath}`);
			...
			this.currentAudio.addEventListener('error', (e) => {
				...
				console.error('Original path:', track.path);
				new Notice(`Failed to play: ${track.name}\nPath: ${track.path}`);
			});
			...
			this.currentAudio.addEventListener('canplay', () => {
				console.log(`Ready to play: ${track.name}`);
			});
			...
			new Notice(`🎵 Now playing: ${track.name}`);
		} catch (error) {
